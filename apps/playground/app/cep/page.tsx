...
export default function CepPlaygroundPage() {
  ...
  const cliCommand = input ? `br-validators cep validate ${stripped || '<value>'}` : '';
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <h1 style={{ fontSize: '1.75rem', margin: '1rem 0 0.5rem' }}>CEP Validator</h1>
      <p style={{ color: '#9aa5bd', marginBottom: '1.5rem' }}>
        8-digit format only · no check digit · does not verify Correios registration
      </p>
      ...
      <p style={{ marginTop: '1.5rem', fontSize: '0.9rem' }}>
        Official source:{' '}
        <a href={CEP_OFFICIAL_SOURCE_URL} target="_blank" rel="noreferrer" style={{ color: '#7aa2ff' }}>
          Correios Busca CEP manual
        </a>
      </p>
