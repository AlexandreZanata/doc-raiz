...
| **RG (Registro Geral)** | Per-state SSP/IGP | Full table: [§ RG](#rg--reference-index) | **UF required** — no federal algorithm. **7/27 UFs:** SP, RJ, MG, PR, RS, SC, BA. SP/RJ/MG: Ghiorzi DV tables; PR/RS/BA: format-only; SC: format-only + mask. Vectors: `rg.{sp,rj,mg,pr,rs,sc,ba}.official.json`. |
